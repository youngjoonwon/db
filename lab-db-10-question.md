...
#### Question: Complete the following python code: *yourname-db-lab-10.py*

For Python installation, please do anything you like. (e.g., Visual Studio Code, python.org)
or something like this is also okay: https://dev.to/youngjoonwon/python-install-in-windows-61m
...
# Dataset description
#####################
# 'trade-data.csv' has some cryptocurrency datasets during March. 2024.
...
def q_1():
    ...
    #How many rows (trades) are available from Mar 15 to 31, 2024?
    #print its count. 
...
def q_2():
    ...
    #How many rows exist per day during the entire March?
    #print its counts in sequence (list) in March. If there's a missing day, fill it with -1.  
...
def q_3():
    ...
    #What are the daily sum of 'amount' during the entire March?
    #print its sum in sequence (list). If there's a missing day, fill it with 0.  
...
def q_4():
    ...
    #Repeat q_2, but this time let's count them weekly (not by daily) and separately where 			the side is equal to 0 or 1.
    #Don't print them in sequqence. Instead, save a figure ('yourname.png') for weekly row 			counts, using a stacked bar of side 0 and side 1.
...
- 'speed' should be where trade is 0. 'lifespan' should be where trade is 1. 
...
- Submit it to LMS (check the deadline), .py file only (db-lab-10-question-submission).
- DO NOT put any space in your submit file name. e.g.) 'young  db-lab-10 .py' (X). 0 will be given.
